...
from torch_geometric.nn.conv.utils.helpers import expand_left  # CB
from torch_geometric.nn.conv.utils.inspector import Inspector, func_body_repr, func_header_repr  # CB
from torch_geometric.nn.conv.utils.jit import class_from_module_repr  # CB
...
class MessagePassing(torch.nn.Module):
    ...
    def propagate(self, edge_index: Adj, size: Size = None, **kwargs):
        ...
        for hook in self._propagate_forward_pre_hooks.values():  # Executed 0 times, CB
        ...
        if (isinstance(edge_index, SparseTensor) and self.fuse
                and not self.__explain__):  # False, CB
        ...
        # Otherwise, run both functions in separation.
        elif isinstance(edge_index, Tensor) or not self.fuse:  # True
            if decomposed_layers > 1:  # False, CB
            ...
            for i in range(decomposed_layers):
                if decomposed_layers > 1:  #  Always False, CB
                ...
                for hook in self._message_forward_pre_hooks.values():  # Not exec, CB
                    res = hook(self, (msg_kwargs, ))
                    if res is not None:
                        msg_kwargs = res[0] if isinstance(res, tuple) else res
                out = self.message(**msg_kwargs)  # Not multiplied with weights, CB

                for hook in self._message_forward_hooks.values():  # Not exec, CB
                ...
                for hook in self._aggregate_forward_pre_hooks.values():  # Not exec, cb
                    res = hook(self, (aggr_kwargs, ))
                    if res is not None:
                        aggr_kwargs = res[0] if isinstance(res, tuple) else res
                out = self.aggregate(out, **aggr_kwargs)
                for hook in self._aggregate_forward_hooks.values():  # Not exec, CB
                    res = hook(self, (aggr_kwargs, ), out)
                    if res is not None:
                        out = res

                update_kwargs = self.inspector.distribute('update', coll_dict)  # No elem, CB
                out = self.update(out, **update_kwargs)
        ...
        for hook in self._propagate_forward_hooks.values():  # No exec, CB
