...
from models_message_passing import MessagePassing  # CB
...
class LabelPropagation(MessagePassing):
    ...
    def forward(
        ...
        """"""
        if y.dtype == torch.long:  # False, CB
            y = F.one_hot(y.view(-1)).to(torch.float)

        out = y
        if mask is not None:  # False, CB
            out = torch.zeros_like(y)
            out[mask] = y[mask]

        if isinstance(edge_index, SparseTensor) and not edge_index.has_value():  # False, CB
            edge_index = gcn_norm(edge_index, add_self_loops=False)
        elif isinstance(edge_index, Tensor) and edge_weight is None:  # False, CB
